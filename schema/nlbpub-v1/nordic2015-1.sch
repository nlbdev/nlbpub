...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
    ...
    <pattern id="epub_nordic_13_b">
        <rule context="html:*[self::html:section or self::html:article][ancestor::html:body and not(parent::html:body) and not(parent::html:section[tokenize(@epub:type,'\s+')='part'])]">
    ...
    <pattern id="epub_nordic_15">
        <!-- see also nordic2015-1.opf-and-html.sch for multi-document version -->
        <rule context="html:body/html:*[self::html:section or self::html:article]">
        ...
        <rule context="html:figure">
            <report
                test="ancestor::html:a        or ancestor::html:abbr    or ancestor::html:a[tokenize(@epub:type,' ')='annoref']   or
                          ...
                          ancestor::html:address  or ancestor::html:*[tokenize(@epub:type,' ')='z3998:author']     or ancestor::html:p[tokenize(@epub:type,' ')='bridgehead'] or ancestor::html:*[tokenize(@class,' ')='byline']    or
                          ancestor::html:cite     or ancestor::html:*[tokenize(@epub:type,' ')='covertitle'] or ancestor::html:*[tokenize(@class,' ')='dateline']   or
    ...
    <pattern id="epub_nordic_26_a">
        <rule context="html:*[ancestor::html:body and tokenize(@epub:type,'\s+')=('note','rearnote','endnote','footnote')]">
    ...
    <pattern id="epub_nordic_26_b">
        <rule context="html:a[ancestor::html:body and tokenize(@epub:type,'\s+')='noteref']">
            <!-- this is the single-HTML version of the rule; the multi-HTML version of this rule is in nordic2015-1.opf-and-html.sch -->
    ...
    <pattern id="epub_nordic_27_a">
        <rule context="html:*[ancestor::html:body and tokenize(@epub:type,' ')='annotation']">
            <!-- this is the single-HTML version of the rule; the multi-HTML version of this rule is in nordic2015-1.opf-and-html.sch -->
    ...
    <pattern id="epub_nordic_27_b">
        <rule context="html:a[ancestor::html:body and tokenize(@epub:type,'\s+')='annoref']">
            <!-- this is the single-HTML version of the rule; the multi-HTML version of this rule is in nordic2015-1.opf-and-html.sch -->
    ...
    <pattern id="epub_nordic_29c">
        <rule
            context="html:*[tokenize(@epub:type,' ')='z3998:production'][ancestor::html:a        or ancestor::html:abbr    or ancestor::html:a[tokenize(@epub:type,' ')='annoref']   or
                                     ...
                                     ancestor::html:p[tokenize(@class,' ')='line']       or ancestor::html:p]">
            <report
                test="descendant::html:*[self::html:address    or self::html:aside[tokenize(@epub:type,' ')='annotation'] or self::html:*[tokenize(@epub:type,' ')='z3998:author']   or
                                           ...
                                           self::html:h1[tokenize(@epub:type,' ')='fulltitle']   or
    ...
    <pattern id="epub_nordic_128_a">
        <rule context="html:meta[boolean(@name) and contains(@name, ':') and not(substring-before(@name, ':') = ('epub', 'dc', 'dcterms', 'marc', 'media', 'onix', 'xsd'))]">
            ...
            <let name="namespace" value="if ($prefix = 'nordic') then 'http://www.mtm.se/epub/' else
                                         ...
                                         if ($prefix = 'nlbprod') then 'http://www.nlb.no/production' else
                                         ''"/>
            <assert test="matches(string(ancestor::html:html[1]/@epub:prefix[1]), concat('(^|\s)', $prefix, ': *[^\s]+(\s|$)'))">[nordic128a] on the html element: the epub:prefix attribute must declare the '<value-of select="$prefix"/>' prefix (epub:prefix="<value-of select="$prefix"/>: <value-of select="if ($namespace) then $namespace else 'URI'"/>")</assert>
        ...
        <rule context="*[@epub:type]">
            ...
            <let name="prefixes-with-namespace" value="for $prefix in $prefixes return (
                                                            ...
                                                            if ($prefix = 'schema') then concat($prefix, ': ', 'http://schema.org/') else
                                                            if ($prefix = 'nlbprod') then concat($prefix, ': ', 'http://www.nlb.no/production') else
                                                            ())"/>
    ...
    <pattern id="epub_nordic_128_c">
        <rule context="html:meta[@name='nordic:guidelines']">
    ...
    <pattern id="epub_nordic_204_a">
        <rule context="html:*[tokenize(@epub:type,'\s+')='footnote']">
            <assert test="(ancestor::html:section | ancestor::html:body)[tokenize(@epub:type,'\s+')='footnotes']">[nordic204a] 'footnote' must have a section<value-of
                    select="if (ancestor::html:body) then '' else ' or body'"/> ancestor with 'footnotes': <value-of
                    select="concat('&lt;',name(),string-join(for $a in (@*) return concat(' ',$a/name(),'=&quot;',$a,'&quot;'),''),'&gt;')"/></assert>
        </rule>
    </pattern>
    ...
    <pattern id="epub_nordic_268">
        <rule context="html:h1 | html:h2 | html:h3 | html:h4 | html:h5 | html:h6">
            ...
            <let name="this-level" value="xs:integer(replace(name(),'.*(\d)$','$1'))"/>
    ...
    <pattern id="epub_nordic_270">
        <rule context="html:p[tokenize(@epub:type,'\t+')='bridgehead']">
            <assert test="parent::html:body | parent::html:section | parent::html:article | parent::html:div">[nordic270] Bridgehead is only allowed as a child of section, article and div: <value-of
                    select="concat('&lt;',name(),string-join(for $a in (@*) return concat(' ',$a/name(),'=&quot;',$a,'&quot;'),''),'&gt;')"/></assert>
        </rule>
    </pattern>
    ...
    <pattern id="epub_nordic_275">
        <rule context="html:img[@longdesc and ancestor::html:body]">
